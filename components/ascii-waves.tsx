...
const Scene: React.FC<SceneProps> = ({
  mouse, characters, color, invert, scale, size: elementSize,
  speed, hasMouse, intensity, interactionIntensity, waveTension, waveTwist,
}) => {
  ...
  useEffect(() => {
    ...
    return () => { texture.dispose(); fontTextureRef.current = null; };
  }, [safeCharacters]);
  ...
  useFrame((state) => {
    ...
    m.uResolution!.value.set(size.width, size.height);
    m.uColor!.value.set(color);
    m.uInvert!.value = invert;
    m.uScale!.value = scale;
    m.uSize!.value = elementSize;
    m.uSpeed!.value = speed;
    m.uHasMouse!.value = hasMouse ? 1.0 : 0.0;
    m.uIntensity!.value = intensity;
    m.uInteractIntensity!.value = interactionIntensity;
    m.uWaveTension!.value = waveTension;
    m.uWaveTwist!.value = waveTwist;
    m.uCharCount!.value = safeCharacters.length;
    if (fontTextureRef.current) m.uFontTexture!.value = fontTextureRef.current;
    if (mouse.current) m.uMouse!.value.lerp(mouse.current, 0.1);
  });
